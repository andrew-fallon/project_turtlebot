...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        for px in range(-1,2):
            for py in range(-1,2):
                if (px,py) != (0,0):
                    negh = (x[0]+px*self.resolution,x[1]+py*self.resolution)
                    negh = self.snap_to_grid(negh)
                    if self.is_free(negh):
                        neighbors.append(negh)
        return neighbors
    ...
    def solve(self):
        while len(self.open_set)>0:
            ...
            # Get the neghibor list and loop over them
            neghibors = self.get_neighbors(x_cur)
            for neghibor in neghibors:
                if neghibor in self.closed_set:
                    continue
                g_prime = self.g_score[x_cur] + self.distance(x_cur,neghibor)
                if neghibor not in self.open_set:
                    self.open_set.append(neghibor)
                elif g_prime > self.g_score[neghibor]:
                    continue
